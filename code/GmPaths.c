...
bool PathFinding(GmPathContext *context, GmPos src_pos, GmPos dst_pos, WaypointArray *waypoints)
{
    ...
    while (PathHeapPop(&context->prioq, &top)) {
        ...
        float curr_dist = DistToTrap(dst_pos, curr_node->point.trap);
        if (curr_dist < closest_point_dist) {
            closest_point_dist = curr_dist;
            closest_point.trap = curr_node->point.trap;
            closest_point.pos.plane = curr_node->point.pos.plane;
        }
    ...
    if (closest_point.trap && IsPointInTrap(dst_pos, closest_point.trap)) {
        ...
        PathCreateWaypoints(context, src_point, closest_point, waypoints);
    }
