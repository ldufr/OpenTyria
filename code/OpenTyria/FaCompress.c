...
int SetupNodesandTree(DecompressState* State, HuffmanData* HData)
{
    int err = 0;

    if (HData->CodesForLargeEncoding)
        free(HData->CodesForLargeEncoding);
    ...
    HData->CodesForLargeEncoding = NULL;
    ...
    {
        unsigned int idx;
        for (idx = 0; idx < ARRAY_SIZE(Table1); ++idx) {
        ...
        unsigned int temp = Table2[Table1[idx].LastIndex - offset];
        unsigned int numberOfCode = temp >> 5; // This is the number of code of this length
        unsigned int codeLen = temp & 0x1f;    // This is the length of this code 
        ...
        if ((codeLen != 0) || SymbolsCount < 2)
        {
            ...
            for (unsigned int i = 0; i < numberOfCode + 1; ++i) {
    ...
    for (unsigned int encodingLen = 0; encodingLen < 9; ++encodingLen)
    {
        ...
        while (currentSymbol != 0xFFFFFFFF)
        {
            ...
            if ((1u << encodingLen) <= nextBitEncoding)
                goto cleanup;
...
int FaDecompress(unsigned char *Input, size_t InputSize, size_t OutputSize, array_uint8_t *result)
{
    int err = 0;

    DecompressState State = {0};
    State.CurInputData = (uint32_t *)Input;
    State.InputDataEnd = (uint32_t *)(Input + InputSize);
    ...
    unsigned char* pOutputBegin = array_push(result, OutputSize);
    memset(pOutputBegin, 0, OutputSize);

    unsigned char* ptrOutput = pOutputBegin;
    unsigned char* ptrOutputEnd = pOutputBegin + OutputSize;
    ...
    {
        if ((err = SetupNodesandTree(&State, &LiteralCodeTable)) != 0)
            goto cleanup;
        if ((err = SetupNodesandTree(&State, &DistanceCodeTable)) != 0)
            goto cleanup;
        ...
        for (unsigned int loopCount = 0; loopCount < blockSize; ++loopCount)
        {
            ...
            else
            {
                ...
                assert((ptrdiff_t)backtrack < (ptrdiff_t)(ptrOutput - pOutputBegin));
